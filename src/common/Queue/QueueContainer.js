...
import { setPlaying, toggleLoop, toggleRandom } from '../../store/actions/playerActions';
import { setLoading, setQueue, setQueueData } from '../../store/actions/queueAction';
import { setPlayingSong, setSongData } from '../../store/actions/songDataAction';
import { QueueListComponent } from './QueueListComponent';

export const QueueContainer = ({ openQueue }) => {
	const apiUrl = process.env.REACT_APP_API_URL;
	const authToken = useSelector((state) => state.authReducer.token);
	const { queue, queueData, loading, loop, random } = useSelector((state) => state.queueReducer);
