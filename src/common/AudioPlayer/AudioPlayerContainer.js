    ...
    const prevSong = () => {
        ...
        if (random) {
            if (randomQueue.indexOf(songData.songId) === 0) {
                ...
                setPlayingSong(dispatch, songId, authToken);
            }
        } else {
            if (queue.indexOf(songData.songId) === 0) {
                setClickedTime(0);
            } else {
                let songId = queue[queue.indexOf(songData.songId) - 1];
                setPlayingSong(dispatch, songId, authToken);
            }
        }
    };
    ...
    const nextSong = () => {
        ...
        const next = (q) => {
            if (q.indexOf(songData.songId) === q.length - 1) {
                setPlayingSong(dispatch, loop ? q[0] : "", authToken);
                if (!loop) dispatch(setPlaying(false));
            } else {
                let songId = q[q.indexOf(songData.songId) + 1];
                setPlayingSong(dispatch, songId, authToken);
            }
        };
